...
impl SearchItem for ModelSearchItem {
    ...
    fn render_details(&self, app: &AppContext) -> Option<Box<dyn Element>> {
        ...
        if self.disable_reason.as_ref() == Some(&DisableReason::RequiresUpgrade) {
            ...
            let mut text_fragments = vec![
                FormattedTextFragment::plain_text(format!("{display_name} 不适用于免费用户。")),
                FormattedTextFragment::hyperlink("升级", upgrade_url),
            ];

            if byok_available {
                text_fragments.push(FormattedTextFragment::plain_text("或".to_string()));
    ...
    fn accessibility_label(&self) -> String {
        let mut label = format!("模型：{}", self.display_text);
        if self.is_selected {
            label.push_str(" (已选择)");
        }
        if self.is_disabled() {
