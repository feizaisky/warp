...
impl PaneGroup {
    ...
    /// Iterate over the file (markdown) notebook panes in this pane group.
    pub fn file_panes<'a>(
        &'a self,
        app: &'a AppContext,
            ...
            Banner::<PaneGroupAction>::new_permanently_dismissible(
                BannerTextContent::formatted_text(vec![
                    FormattedTextFragment::plain_text(
                        "Warp 目前不支持你的默认 Shell，将回退到 zsh。  ",
                    ),
                    FormattedTextFragment::hyperlink("了解更多", WARP_SHELL_COMPATIBILITY_DOCS),
                ]),
            )
        });
