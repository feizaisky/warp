...
pub const BLOCKED_ACTION_MESSAGE_FOR_SEARCHING_CODEBASE: &str = "是否授权访问以下仓库？";
                    ...
                    && active_block.agent_interaction_metadata().is_some()
                {
                    if action.is_none() {
                        ...
                        if let Some(remaining) = next_check_remaining {
                            ...
                            let suffix = format!(" · 下次检查还有 {formatted}");
    ...
    Hoverable::new(props.button_handle.clone(), move |state| {
        ...
        if state.is_hovered() {
            let tool_tip = ui_builder
                .tool_tip("让智能体立即检查此命令，跳过其定时器。".to_owned())
                .build()
                .finish();
...
pub fn render_failed_output(props: FailedOutputProps, app: &AppContext) -> Box<dyn Element> {
    ...
    let error_text = match props.error {
        RenderableAIError::QuotaLimit => {
            ...
            format!(
                "{ERROR_APOLOGY_TEXT}\n\n你已达到额度上限。额度将在 {formatted_next_refresh_time} 重置。",
            )
        }
        RenderableAIError::ServerOverloaded => "Warp 当前负载过高。请稍后重试。".to_string(),
...
fn render_invalid_api_key_error(
    ...
    let alert_text = Text::new("提供的 API key 无效", appearance.ui_font_family(), 14.)
    ...
    let detail_text = Text::new(
        format!(
            "使用 {model_name} 时未能通过 {provider} 认证。\
                     请再次检查你的 API key 是否正确。"
        ),
        appearance.ui_font_family(),
        14.,
    )
