...
impl TipsView {
    ...
    pub fn new(
        ...
        let tip_items = vec![
            TipItem::new(
                "命令面板".to_string(),
                "无需双手离开键盘，即可轻松发现 Warp 中的所有功能。".to_string(),
            ...
            TipItem::new(
                "历史搜索".to_string(),
                "查找、编辑并重新运行以前执行过的命令。".to_string(),
                TipAction::HistorySearch,
                ctx,
            ),
            TipItem::new(
                "AI 命令搜索".to_string(),
                "用自然语言生成 shell 命令。".to_string(),
                TipAction::AiCommandSearch,
                ctx,
            ),
            TipItem::new(
                "主题选择器".to_string(),
                "选择内置主题，或创建您自己的主题，让 Warp 独具一格。".to_string(),
        ...
        if let Some(keystroke) = tip_item.shortcut {
            let shortcut = Flex::row()
                .with_child(
                    Container::new(
                        ui_builder
                            .wrappable_text("快捷键".to_string(), false)
                            .with_style(UiComponentStyles {
                                font_family_id: Some(appearance.ui_font_family()),
                                font_size: Some(appearance.monospace_font_size() * 0.8),
                                ..Default::default()
    ...
    fn render_completed_overlay(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let close_button = ui_builder
            ...
            .with_centered_text_label("关闭欢迎提示".to_string())
            .build()
            .on_click(|ctx, _, _| ctx.dispatch_typed_action(TipsAction::DismissTips))
            .finish();
