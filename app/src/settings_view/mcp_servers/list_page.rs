...
const DESCRIPTION_TEXT: &str = "添加 MCP 服务器以扩展 Warp 智能体的能力。MCP 服务器通过标准化接口向智能体提供数据源或工具，类似插件。您可以添加自定义服务器或使用预设快速上手常用服务器，也可以在此处找到团队共享的服务器。";
...
const EMPTY_STATE_TEXT: &str = "添加 MCP 服务器后，将在此处显示。";
                ...
                && filtered_file_based_cards.is_empty()
            {
                ...
                if !shared_server_cards.is_empty() {
                    ...
                    let shared_by_text = match team_name {
                        Some(name) => format!("Warp 共享及 {name}"),
                        None => "Warp 共享及其他设备".to_string(),
                    };
        ...
        for provider in MCPProvider::iter() {
            ...
            let server_card = ServerCardView::new(
                ...
                installation
                    ...
                    .description
                    .clone()
                    .or_else(|| Some("从配置文件检测到".to_string())),
