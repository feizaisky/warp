...
impl PlatformPageView {
    ...
    pub fn new(ctx: &mut ViewContext<PlatformPageView>) -> Self {
        ...
        let create_api_key_modal_view = ctx.add_typed_action_view(|ctx| {
            Modal::new(Some("新建 API 密钥".to_string()), create_api_key_body, ctx)
    ...
    fn show_create_api_key_modal(&mut self, ctx: &mut ViewContext<Self>) {
        // Ensure header reads "新建 API 密钥" when opening the form
        self.create_api_key_modal_state
            .set_title(Some("新建 API 密钥".to_string()), ctx);
    ...
    fn handle_create_api_key_modal_event(
        ...
        match event {
            ...
            CreateApiKeyModalEvent::Created { api_key } => {
                ...
                self.create_api_key_modal_state
                    .set_title(Some("保存您的密钥".to_string()), ctx);
...
impl PlatformPageWidget {
    fn render_description_with_link(
        ...
        let text = vec![
            FormattedTextFragment::plain_text("创建和管理 API 密钥，以允许其他 Oz Cloud 智能体访问您的 Warp 账户。\n更多信息，请访问 "),
    ...
    fn render_api_key_row(
        ...
        let expires_at = key
            .expires_at
            .map(|dt| format!("{}", dt.format("%Y年%-m月%-d日")))
            .unwrap_or_else(|| "从未".to_owned());
