        ...
        match command_context {
            ...
            CommandContext::LastRunCommand {
                ...
                hint_text: match mins_since_completion {
                    Some(mins) if mins >= 60 => "1 小时前完成".to_string(),
                    Some(mins) if mins == 1 => format!("{mins} 分钟前完成"),
                    Some(mins) => format!("{mins} 分钟前完成"),
                    None => "未找到时间戳".to_string(),
                },
            },
