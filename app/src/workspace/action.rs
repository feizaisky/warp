...
impl From<&WorkspaceAction> for LoginGatedFeature {
    fn from(val: &WorkspaceAction) -> LoginGatedFeature {
        use WorkspaceAction::*;
        match val {
            ImportToTeamDrive => "导入到团队云盘",
            CreateTeamNotebook => "创建团队笔记本",
            CreateTeamWorkflow => "创建团队工作流",
            CreateTeamFolder => "创建团队文件夹",
            CreateTeamEnvVarCollection => "创建团队环境变量集合",
            CreateTeamAIPrompt => "创建团队提示词",
            OpenShareSessionModal(_) => "共享会话",
            _ => "未知原因",
        }
    }
}
