...
const UPDATE_READY_TEXT: &str = "更新 Warp";
...
const SESSION_CONFIG_TAB_CONFIG_CHIP_TEXT: &str = "在此访问您的标签页配置。";
...
impl Workspace {
    ...
    fn copy_shared_session_link(
        ...
        self.toast_stack.update(ctx, |toast_stack, ctx| {
            let toast = DismissibleToast::default("远程控制链接已复制。".to_string());
            toast_stack.add_ephemeral_toast(toast, ctx);
        });
    }
    ...
    fn show_header_toolbar_context_menu(
        ...
        let items = vec![MenuItemFields::new("重新排列工具栏项")
    ...
    fn view_logs(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(
            ...
            |me, result, ctx| match result {
                ...
                Ok(Err(err)) => {
                    let error_message = format!("创建日志包失败: {err}");
    ...
    fn unified_new_session_menu_items(
        ...
        if is_any_ai_enabled {
            let mut agent_item = MenuItemFields::new("智能体")
        ...
        if FeatureFlag::LocalDockerSandbox.is_enabled() {
            let mut docker_item = MenuItemFields::new("本地 Docker 沙盒")
        ...
        if FeatureFlag::TabConfigs.is_enabled() {
            ...
            menu_items.push(
                MenuItemFields::new_submenu("新建工作树配置")
    ...
    pub fn toggle_tab_bar_overflow_menu(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if FeatureFlag::Autoupdate.is_enabled() && ChannelState::show_autoupdate_menu_items() {
            if let Some(version) = ChannelState::app_version() {
                menu_items.push(
                    MenuItemFields::new(format!("当前版本为 {version}"))
                        .with_disabled(true)
                        .into_item(),
                );
                match autoupdate::get_update_state(ctx) {
                    ...
                    | AutoupdateStage::UpdatedPendingRestart { new_version } => menu_items.push(
                        MenuItemFields::new(format!("安装更新 ({})", new_version.version))
                            .with_on_select_action(WorkspaceAction::ApplyUpdate)
                            .into_item(),
                    ),
                    AutoupdateStage::Updating { new_version, .. } => menu_items.push(
                        MenuItemFields::new(format!("正在更新至 ({})", new_version.version))
                            .with_disabled(true)
                            .into_item(),
                    ),
    ...
    fn install_cli(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.spawn(
            ...
            |view, result, ctx| match result {
                ...
                Err(error) => {
                    let error_message = format!("安装 Oz 命令失败: {error}");
                    log::error!("{error_message}");
                    view.toast_stack.update(ctx, |toast_stack, ctx| {
                        let toast = DismissibleToast::error(error_message);
                        toast_stack.add_persistent_toast(toast, ctx);
                    ...
                    ConstrainedBox::new(
                        Container::new(
                            Flex::row()
                                ...
                                .with_cross_axis_alignment(CrossAxisAlignment::Center)
                                .with_child(
                                    Text::new_inline(" + 添加新仓库", font_family, font_size)
        ...
        ctx.spawn(future, move |_workspace, conversation, ctx| {
            let Some(conversation) = conversation else {
                ...
                WorkspaceToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    let toast = DismissibleToast::error("加载对话失败。".to_owned());
    ...
    fn show_fork_toast(
        ...
        WorkspaceToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
            let toast = DismissibleToast::default(format!("已分叉 \"{title}\""));
            toast_stack.add_ephemeral_toast(toast, window_id, ctx);
        });
    }
            ...
            ctx.request_desktop_notification_permissions(move |view, outcome, ctx| {
                match &outcome {
                    ...
                    RequestPermissionsOutcome::PermissionsDenied => {
                        ...
                        view.toast_stack.update(ctx, |toast_stack, ctx| {
                            let toast = DismissibleToast::error(
                                "Warp 没有发送桌面通知的权限。".to_string(),
                            )
                            .with_link(ToastLink::new("排查通知问题".to_string()).with_href(url));
                            toast_stack.add_persistent_toast(toast, ctx);
                        });
                    }
                ...
                if let Some(version) = ChannelState::app_version() {
                    ...
                    if FeatureFlag::AvatarInTabBar.is_enabled() {
                        self.update_toast_stack.update(ctx, |stack, ctx| {
                            ...
                            let mut link = ToastLink::new("查看更新日志".to_owned())
                                .with_onclick_action(WorkspaceAction::ViewLatestChangelog);
                            if let Some(keystroke) = keystroke {
                                link = link.with_keystroke(keystroke);
                            }

                            let toast = DismissibleToast::default(String::from("Warp 已更新！"))
                                .with_link(link);
                        ...
                        .update(ctx, |view, ctx| match result.success_type {
                            OperationSuccessType::Success => {
                                ...
                                if result.operation == ObjectOperation::Trash {
                                    new_toast = new_toast.with_link(
                                        ToastLink::new("撤销".to_string()).with_onclick_action(
    ...
    fn render_new_session_button(&self, ctx: &AppContext) -> Box<dyn Element> {
        ...
        let new_tab_tool_tip_label_text = "新建标签页".to_string();
    ...
    fn render_web_anonymous_user_sign_in_button(
        ...
        let button = appearance
            ...
            .with_centered_text_label(String::from("注册"));
    ...
    fn render_offline_button(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let tool_tip_label_text = "离线时部分功能可能不可用".to_string();
        let icon = ConstrainedBox::new(
            Container::new(
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            SampleProcess => {
                ...
                self.toast_stack.update(ctx, |view, ctx| {
                    view.add_ephemeral_toast(
                        DismissibleToast::default("采样进程持续 3 秒...".to_string()),
                        ctx,
                    );
                });
            ...
            ExecuteDeleteConversation {
                ...
                ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                    toast_stack.add_ephemeral_toast(
                        DismissibleToast::success("对话已删除".to_string()),
                        window_id,
                        ctx,
                    );
                });
