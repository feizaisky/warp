...
/// flight.
const GENERATING_PLACEHOLDER_TEXT: &str = "正在生成提交信息\u{2026}";
...
const FALLBACK_PLACEHOLDER_TEXT: &str = "请输入提交信息";
...
/// ran.
const LOADING_LABEL: &str = "正在提交\u{2026}";
...
pub(super) fn new_state(
    ...
    let commit_and_create_pr_button = if allow_create_pr {
        Some(ctx.add_typed_action_view(|_ctx| {
            ActionButton::new("提交并创建 PR", SecondaryTheme)
                .with_size(ButtonSize::XSmall)
                .with_height(32.)
        ...
        move |_me, result, ctx| {
            ...
            match result {
                Ok(CommitOutcome::Committed) => {
                    show_toast("更改已成功提交。", ctx);
                }
                Ok(CommitOutcome::Pushed) => {
                    show_toast("更改已提交并推送。", ctx);
                }
